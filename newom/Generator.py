...
class OmniglotGenerator(object):
    ...
    def __init__(self, data_folder, batch_size=1, nb_samples=5, nb_samples_per_class=10, max_rotation=-np.pi/6, max_shift=10, img_size=(20,20), max_iter=None,train_samples_per_class=1):
    ...
    def sample(self, nb_samples):
        ...
        example_outputs = np.zeros((self.batch_size, nb_samples * self.nb_samples_per_class), dtype=np.float32)  
    ...
    def sample1(self, nb_samples):
        sampled_character_folders = random.sample(self.character_folders, nb_samples)
        random.shuffle(sampled_character_folders)

        example_inputs = np.zeros((self.batch_size, nb_samples * self.nb_samples_per_class, np.prod(self.img_size)), dtype=np.float32)
        example_outputs = np.zeros((self.batch_size, nb_samples * self.nb_samples_per_class,nb_samples) ,dtype=np.float32)    
        trainone_inputs = np.zeros((self.batch_size, nb_samples * self.train_samples_per_class, np.prod(self.img_size)), dtype=np.float32)
        trainone_outputs = np.zeros((self.batch_size, nb_samples * self.train_samples_per_class,nb_samples) ,dtype=np.float32) #notice hardcoded np.float32 here and above, change it to something else in tf
        ...
        for i in range(self.batch_size):
            ...
            labelone=np.zeros((nb_samples * self.nb_samples_per_class,nb_samples))
            labelk=np.asarray(labels, dtype=np.int32)
            for j in range(nb_samples * self.nb_samples_per_class):
                labelone[j][int(labelk[j])]=1
            ...
            example_inputs[i] = np.asarray([load_transform(filename, angle=angle, s=shift, size=self.img_size).flatten() \
                                            for (filename, angle, shift) in zip(image_files, angles, shifts)], dtype=np.float32)
            example_outputs[i] = np.asarray(labelone, dtype=np.int32)
            
        for i in range(self.batch_size):
            labels_and_images = get_shuffled_images(sampled_character_folders, range(nb_samples), nb_samples=self.train_samples_per_class)
            sequence_length = len(labels_and_images)
            labels, image_files = zip(*labels_and_images)
            
            labelone=np.zeros((nb_samples * self.train_samples_per_class,nb_samples))
            labelk=np.asarray(labels, dtype=np.int32)
            for j in range(nb_samples * self.train_samples_per_class):
                labelone[j][int(labelk[j])]=1
            
            angles = np.random.uniform(-self.max_rotation, self.max_rotation, size=sequence_length)
            shifts = np.random.uniform(-self.max_shift, self.max_shift, size=sequence_length)

            trainone_inputs[i] = np.asarray([load_transform(filename, angle=angle, s=shift, size=self.img_size).flatten() \
                                            for (filename, angle, shift) in zip(image_files, angles, shifts)], dtype=np.float32)
            trainone_outputs[i] = np.asarray(labelone, dtype=np.int32)
            
        return example_inputs, example_outputs,trainone_inputs,trainone_outputs
